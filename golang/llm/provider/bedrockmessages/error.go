...
func mapError(err error, profileName string) *provider.Error {
	...
	return &provider.Error{Code: provider.CodeProviderUnavailable, Phase: provider.PhaseDispatch, Dispatch: provider.DispatchAmbiguous, Retry: provider.RetrySameOperation, SafeMessage: "provider request failed before a response was classified", SafeDetails: map[string]string{"provider": profileName}, Cause: err}
}
	...
	if apiErr.Response != nil {
		for _, header := range []string{"x-amzn-requestid", "x-amzn-request-id", "request-id", "x-request-id"} {
			if mapped.Provider.RequestID == "" {
				mapped.Provider.RequestID = apiErr.Response.Header.Get(header)
			}
		}
	}
	return mapped
