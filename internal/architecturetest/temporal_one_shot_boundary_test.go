...
func TestTemporalOneShotActivityBoundary(t *testing.T) {
	...
	activities := parseGoFile(t, filepath.Join(root, "activity", "activities.go"))
	if err := validateActivitiesOneShotBoundary(activities); err != nil {
		t.Fatal(err)
	}

	if err := validateNoTemporalStreamingReferences(root); err != nil {
		t.Fatal(err)
	}

	runtime := parseGoFile(t, filepath.Join(root, "internal", "runtime", "runtime.go"))
...
func TestTemporalOneShotActivityBoundaryRejectsStreamingFixtures(t *testing.T) {
	root := repositoryRoot(t)
	activitiesPath := filepath.Join(root, "activity", "activities.go")
	activitiesSource := readArchitectureSource(t, activitiesPath)
	runtimePath := filepath.Join(root, "internal", "runtime", "runtime.go")
...
import (
	model "github.com/mfow/llm-temporal-worker/llm"
	adapter "github.com/mfow/llm-temporal-worker/llm/provider"
)
