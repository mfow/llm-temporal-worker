...
let query_response_of_json value =
  ...
  let* cost = match cost_status with
    | "exact" -> let* actual = required "query response" "actual_cost_usd" fields in let* actual = string "query response.actual_cost_usd" actual in let* actual = match Usd_decimal.of_string actual with Ok value -> Ok value | Error message -> Error (errorf "query response.actual_cost_usd: %s" message) in let* method_ = required "query response" "cost_method" fields >>= string "query response.cost_method" in let method_ = match method_ with "provider_reported" -> Provider_reported | "catalog_usage" -> Catalog_usage | "control_query_zero" -> Control_query_zero | _ -> Control_query_zero in Ok (Exact_cost { actual_cost_usd = actual; method_; catalog_version = None })
    | "unknown" -> let* reason = required "query response" "cost_unknown_reason_code" fields >>= string "query response.cost_unknown_reason_code" in let reason = match reason with "provider_did_not_report_cost" -> Provider_did_not_report_cost | "catalog_incomplete" -> Catalog_incomplete | "state_unavailable" -> State_unavailable | "ambiguous_dispatch" -> Ambiguous_dispatch | _ -> State_unavailable in Ok (Unknown_cost { reason })
