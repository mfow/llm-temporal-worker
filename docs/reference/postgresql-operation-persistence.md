...
path instead of surfacing a database duplicate-key error. Reusing an operation
ID with a different scope or operation kind is classified as an operation
conflict even though its idempotency key differs.
